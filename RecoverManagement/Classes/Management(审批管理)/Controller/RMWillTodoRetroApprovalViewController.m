//
//  RMWillTodoRetroApprovalViewController.m
//  RecoverManagement
//
...
@interface RMWillTodoRetroApprovalViewController ()<UIScrollViewDelegate>
{
...
@implementation RMWillTodoRetroApprovalViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.titleLB.text = [NSString stringWithFormat:@"我的待办-%@审批",_state];
    [self addLeftView:@""];
...
- (void)setDocumentsView{
    ...
    NSArray *firstTitleArr = @[@"申请人    ：",@"申请编号：",@"申请时间："];
    NSArray *firstResultsArr = @[@"张三",@"BQ-2016-0001",@"2016-09-01  14:05"];
    NSArray *secondTitleArr = @[[NSString stringWithFormat:@"%@日期：",_state],[NSString stringWithFormat:@"%@地点：",_state],[NSString stringWithFormat:@"%@时间：",_state],[NSString stringWithFormat:@"%@说明：",_state]];
    ...
    mainScrollView.frame  = CGRectMake(0, 64+45, KScreenWidth, KScreenHeight-109-50);
    ...
    NSArray *buttonArr = @[@"转审",@"否决",@"通过"];
    for (int i = 0; i<buttonArr.count; i++) {
        UIButton *bottomBtn = [[UIButton alloc]initWithFrame:CGRectMake(KScreenWidth-83*(3-i), 0, 85, 50)];
        [bottomBtn setTitle:buttonArr[i] forState:UIControlStateNormal];
        [bottomBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        if (i==0) {
            bottomBtn.backgroundColor = LMColor(0, 148, 214);
        }else if (i==1) {
...
- (void)bottomBtnClick:(UIButton *)sender{
    NSLog(@"%d。。。。。。。",sender.tag);
    if (sender.tag == 320) {
        RMTurnOnViewController *vc = [[RMTurnOnViewController alloc]init];
...
- (void)setShenPiView{
    ...
    NSArray *firstTitleArr = @[@"发起人    ：",@"申请编号："];
    NSArray *firstResultsArr = @[@"张三",@"BQ-2016-0001"];
...
- (void)titleBtnClick:(UIButton *)sender{
    ...
    if (willOrDoTag==0) {
        ...
        mainScrollView.frame  = CGRectMake(0, 64+45, KScreenWidth, KScreenHeight-109);
