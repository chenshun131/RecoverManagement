...
typedef NS_ENUM(NSUInteger, RMDragCellCollectionViewScrollDirection) {
    RMDragCellCollectionViewScrollDirectionNone = 0,
    RMDragCellCollectionViewScrollDirectionLeft,
    RMDragCellCollectionViewScrollDirectionRight,
    RMDragCellCollectionViewScrollDirectionUp,
    RMDragCellCollectionViewScrollDirectionDown
};

@interface RMDragCellCollectionView ()<UIGestureRecognizerDelegate>
...
@property (nonatomic, assign) RMDragCellCollectionViewScrollDirection scrollDirection;
@property (nonatomic, assign) CGFloat oldMinimumPressDuration;
@property (nonatomic, assign, getter=isObservering) BOOL observering;

@end

@implementation RMDragCellCollectionView
@dynamic delegate;
@dynamic dataSource;
...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(nonnull UICollectionViewLayout *)layout
{
    ...
    if (self) {
        [self rm_initializeProperty];
        [self rm_addLongPressGesture];
        [self rm_addDoubleTapGesture];
    }
    return self;
}
 ...
 *  添加一个自定义的滑动手势
 */
- (void)rm_addLongPressGesture{
    UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(rm_longPressed:)];
...
- (void)rm_addDoubleTapGesture{
 ...
 [_longPressGesture   requireGestureRecognizerToFail:_doubleTapGesture];
}
/**
 *  监听手势的改变
 */
- (void)rm_longPressed:(UILongPressGestureRecognizer *)longPressGesture{
    if (longPressGesture.state == UIGestureRecognizerStateBegan) {
        [self rm_gestureBegan:longPressGesture];
    }
    if (longPressGesture.state == UIGestureRecognizerStateChanged) {
        [self rm_gestureChange:longPressGesture];
    }
    if (longPressGesture.state == UIGestureRecognizerStateCancelled ||
        longPressGesture.state == UIGestureRecognizerStateEnded){
        [self rm_gestureEndOrCancle:longPressGesture];
    }
}
/**
 *  监听手势
...
- (void)handleDoubleTap:( UITapGestureRecognizer *)doubleTap{
    if ([self.delegate respondsToSelector:@selector(doubleTapCellCollectionView:tapCellIndexPath:)]) {
        [self.delegate doubleTapCellCollectionView:self tapCellIndexPath:[self indexPathForItemAtPoint:[doubleTap locationOfTouch:0 inView:doubleTap.view]]];
    }
}
/**
 *  手势开始
 */
- (void)rm_gestureBegan:(UILongPressGestureRecognizer *)longPressGesture{
    //获取手指所在的cell
...
//    UIView *tempMoveCell = [cell snapshotViewAfterScreenUpdates:NO];
      UIView *tempMoveCell = [self setCellCopiedImage:cell];
    cell.hidden = YES;
    ...
    //开启边缘滚动定时器
    [self rm_setEdgeTimer];
    //开启抖动
    if (_shakeWhenMoveing && !_editing) {
        [self rm_shakeAllCell];
 ...
 *  手势拖动
 */
- (void)rm_gestureChange:(UILongPressGestureRecognizer *)longPressGesture{
    //通知代理
 ...
 *  手势取消或者结束
 */
- (void)rm_gestureEndOrCancle:(UILongPressGestureRecognizer *)longPressGesture{
    UICollectionViewCell *cell = [self cellForItemAtIndexPath:_originalIndexPath];
    self.userInteractionEnabled = NO;
    [self rm_stopEdgeTimer];
    //通知代理
...
- (void)rm_setEdgeTimer{
    if (!_edgeTimer && _edgeScrollEable) {
        _edgeTimer = [CADisplayLink displayLinkWithTarget:self selector:@selector(rm_edgeScroll)];
        [_edgeTimer addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
    }
}

- (void)rm_stopEdgeTimer{
    if (_edgeTimer) {
...
//复制当前移动的Cell
- (UIImageView *)setCellCopiedImage:(UICollectionViewCell *)cell {
...
- (void)rm_edgeScroll{
    [self rm_setScrollDirection];
    switch (_scrollDirection) {
        case RMDragCellCollectionViewScrollDirectionLeft:{
            //这里的动画必须设为NO
            ...
            break;
        case RMDragCellCollectionViewScrollDirectionRight:{
            [self setContentOffset:CGPointMake(self.contentOffset.x + 4, self.contentOffset.y) animated:NO];
            _tempMoveCell.center = CGPointMake(_tempMoveCell.center.x + 4, _tempMoveCell.center.y);
            _lastPoint.x += 4;
            ...
            break;
        case RMDragCellCollectionViewScrollDirectionUp:{
            ...
            break;
        case RMDragCellCollectionViewScrollDirectionDown:{
            [self setContentOffset:CGPointMake(self.contentOffset.x, self.contentOffset.y + 4) animated:NO];
            _tempMoveCell.center = CGPointMake(_tempMoveCell.center.x, _tempMoveCell.center.y + 4);
            _lastPoint.y += 4;
        }
...
- (void)rm_setScrollDirection{
    _scrollDirection = RMDragCellCollectionViewScrollDirectionNone;
    if (self.bounds.size.height + self.contentOffset.y - _tempMoveCell.center.y < _tempMoveCell.bounds.size.height / 2 && self.bounds.size.height + self.contentOffset.y < self.contentSize.height) {
        _scrollDirection = RMDragCellCollectionViewScrollDirectionDown;
    }
    if (_tempMoveCell.center.y - self.contentOffset.y < _tempMoveCell.bounds.size.height / 2 && self.contentOffset.y > 0) {
        _scrollDirection = RMDragCellCollectionViewScrollDirectionUp;
    }
    if (self.bounds.size.width + self.contentOffset.x - _tempMoveCell.center.x < _tempMoveCell.bounds.size.width / 2 && self.bounds.size.width + self.contentOffset.x < self.contentSize.width) {
        _scrollDirection = RMDragCellCollectionViewScrollDirectionRight;
    }
    
    if (_tempMoveCell.center.x - self.contentOffset.x < _tempMoveCell.bounds.size.width / 2 && self.contentOffset.x > 0) {
        _scrollDirection = RMDragCellCollectionViewScrollDirectionLeft;
    }
}

#pragma mark - public methods

- (void)rm_enterEditingModel{
    _editing = YES;
    ...
    if (_shakeWhenMoveing) {
        [self rm_shakeAllCell];
        [self addObserver:self forKeyPath:@"contentOffset" options:NSKeyValueObservingOptionNew context:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(rm_foreground) name:UIApplicationWillEnterForegroundNotification object:nil];
    }
}

- (void)rm_stopEditingModel{
    _editing = NO;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event{
    _doubleTapGesture.enabled = [self indexPathForItemAtPoint:point];

//    _longPressGesture.enabled = [self indexPathForItemAtPoint:point];
  
//    NSLog(@"%d @%@",_longPressGesture.enabled,[self indexPathForItemAtPoint:point]);
