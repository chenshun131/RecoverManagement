...
@interface RMCalendarView ()
@property (nonatomic, strong) UIButton *selectBtn;
@property (nonatomic, strong) NSMutableArray *daysArray;
@end

@implementation RMCalendarView
...
- (void)setupNextAndLastMonthView {
    ...
    [leftBtn setImage:[UIImage imageNamed:@"sanjiaojiantouzuo"] forState:UIControlStateNormal];
    ...
    [rightBtn setImage:[UIImage imageNamed:@"sanjiaojiantouyou"] forState:UIControlStateNormal];
    rightBtn.tag = 2;
...
//    [self addSubview:self.headlabel];
    ...
    for (int i = 0; i < 42; i++) {
        ...
        dayButton.frame = CGRectMake(x, y, itemW-5, itemH-5);
...
- (void)setStyle_Today:(UIButton *)btn
{
    ...
    btn.layer.masksToBounds = YES;
     ...
     [btn setBackgroundImage:self.dateImage forState:UIControlStateNormal];
...
{;
    ...
    for (NSString *str in self.allDaysArr) {
        if ([str isEqualToString:btn.titleLabel.text]) {
            [btn setBackgroundImage:self.allDaysImage forState:UIControlStateNormal];
           
        }
    }
    for (NSString *str in self.partDaysArr) {
        if ([str isEqualToString:btn.titleLabel.text]) {
               [btn setBackgroundImage:self.partDaysImage forState:UIControlStateNormal];
        }
    }
}
...
- (UIColor *)allDaysColor {
    if (!_allDaysColor) {
        ...
        _allDaysColor = [UIColor cyanColor];
    }
...
- (UIColor *)partDaysColor {
    if (!_partDaysColor) {
        _partDaysColor = [UIColor purpleColor];
    }
...
- (UIColor *)weekDaysColor {
    if (!_weekDaysColor) {
        _weekDaysColor = [UIColor orangeColor];
    }
